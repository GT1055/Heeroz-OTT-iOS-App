/*
 * CSHomePageViewController+Extension.swift
 * This class is used as a extension Class For Home Page
 * @category   vplay
...
extension HomePageViewController: UITableViewDelegate, SkeletonTableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if !moviesArray.count.checkDataPresent() {
            ...
            if moviesArray[indexPath.row].categoryBanner.isEmpty == false {
                ...
                let bannerData = moviesArray[indexPath.row].categoryBanner
                let videoURL = bannerData[0].bannerVideoUrl
...
//                    }
//                    currentBannerList = categoryBanner
...
//            }
            ...
            cell.headerTitle.text = self.moviesArray[indexPath.row].categoryName
    ...
    @objc func didTap(currentTap: UITapGestureRecognizer) {
        ...
        if let currrentCell = self.homeTableView.cellForRow(at: currentIndex) as? CSHomeTableViewCell {
...
//            {
//                if self.callmoreinApiFinishedLoading == true
//                {
//                    if let count = totalCategory{
    ...
    func pagerView(_ pagerView: FSPagerView, cellForItemAt index: Int) -> FSPagerViewCell {
        ...
        if pagerView.tag < moviesArray.count {
            if index < moviesArray[pagerView.tag].categoryBanner.count {
                ...
                let tableCell = self.homeTableView.cellForRow(at: indexPath) as? CSHomeTableViewCell
...
//        let cell = self.homeTableView.cellForRow(at: IndexPath(row: pagerView.tag, section: 0)) as? CSHomeTableViewCell
...
//        let bannerData = moviesArray[pagerView.tag].categoryBanner
//        let videoURL = bannerData[targetIndex].bannerVideoUrl
//        let indexPath = IndexPath(row: pagerView.tag, section: 0)
//        let tableCell = self.homeTableView.cellForRow(at: indexPath) as? CSHomeTableViewCell
//        tableCell?.playButtonIcon.isHidden = videoURL.isEmpty
    }
...
//        let bannerData = moviesArray[pagerView.tag].categoryBanner
//        let videoURL = bannerData[pagerView.currentIndex].bannerVideoUrl
//        let indexPath = IndexPath(row: pagerView.tag, section: 0)
//        let tableCell = self.homeTableView.cellForRow(at: indexPath) as? CSHomeTableViewCell
//        tableCell?.playButtonIcon.isHidden = videoURL.isEmpty
    }
...
extension HomePageViewController: UICollectionViewDelegate, SkeletonCollectionViewDataSource,
UICollectionViewDelegateFlowLayout {
    func collectionSkeletonView(_ skeletonView: UICollectionView, cellIdentifierForItemAt indexPath: IndexPath) -> ReusableCellIdentifier {
        return "cell"
    }
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return (self.moviesArray.count != 0) ? self.moviesArray[collectionView.tag].movieList.count : 10
    }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        self.currentIndexPath = IndexPath.init(row: indexPath.row, section: collectionView.tag)
...
extension HomePageViewController: CSAdMobDelegate {
    func didAdDismissed() {
