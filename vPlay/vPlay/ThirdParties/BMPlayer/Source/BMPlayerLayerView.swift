...
open class BMPlayerLayerView: UIView {
    ...
    open func changeWithKeyPlayAsset(asset: AVURLAsset, videoKey: Int64) {
        let header = ["Referer": REFERER,
                      "Title": ("vplayed/" + String(videoKey)).aesDoubleEncryptedString(),
        ...
        urlAsset = AVURLAsset(url: asset.url, options: options)
        let seekTime = player?.currentTime().seconds
        playerItem = AVPlayerItem(asset: urlAsset!)
        player?.automaticallyWaitsToMinimizeStalling = true
        let targetTime: CMTime = CMTimeMake(value: Int64(5), timescale: 1)
        playerItem?.preferredForwardBufferDuration = CMTimeGetSeconds(targetTime)
        player?.replaceCurrentItem(with: playerItem)
        let draggedTime = CMTimeMake(value: Int64(seekTime!), timescale: 1)
        self.player!.seek(to: draggedTime, toleranceBefore: CMTime.zero, toleranceAfter: CMTime.zero)
        play()
    }
    ...
    open func changePlayAsset(asset: AVURLAsset) {
        urlAsset = asset
        let seekTime = player?.currentTime().seconds
        playerItem = AVPlayerItem(asset: urlAsset!)
        player?.automaticallyWaitsToMinimizeStalling = true
