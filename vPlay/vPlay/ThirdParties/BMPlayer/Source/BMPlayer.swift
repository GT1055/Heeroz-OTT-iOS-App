...
open class BMPlayer: UIView {
    ...
    /**
     It sets the sprite image to the control view properties
     */
    open func setPreviewImage(_ spriteImage: UIImageView) {
        controlView.setSpriteImage(spriteImage)
        controlView.isPreviewAvailable = true
    }
    ...
    @objc fileprivate func fullScreenButtonPressed() {
        if UIDevice.current.orientation.isLandscape {
            ...
            controlView.titleLabel.isHidden = true
        } else {
            controlView.updateUI(true)
            controlView.titleLabel.isHidden = false
    ...
    required public init?(coder aDecoder: NSCoder) {
        ...
        if let customControlView = self.storyBoardCustomControl() {
    ...
    fileprivate func initUIData() {
        NotificationCenter.default.addObserver(self, selector: #selector(self.onOrientationChanged),
...
extension BMPlayer: BMPlayerControlViewDelegate {
    public func controlView(controlView: BMPlayerControlView,
       ...
       playerLayer?.changePlayAsset(asset: resource.definitions[index].avURLAsset)
    }
    public func controlView(controlView: BMPlayerControlView,
                            didChooseDefition index: Int, videoKey: Int64) {
        let selectedDefinition = resource.definitions[index].definition
        playerLayer?.changeWithKeyPlayAsset(asset: resource.definitions[0].avURLAsset, videoKey: videoKey)
        playerLayer?.setBitRate(selectedDefinition)
    }
    ...
    public func controlView(controlView: BMPlayerControlView,
        ...
        if let action = BMPlayerControlView.ButtonType(rawValue: button.tag) {
            ...
            case .replay:
                isPlayToTheEnd = false
                seek(0) {
                    self.controlView.timeSlider.value = 0
                    self.controlView.currentTimeLabel.text = "00:00"
                    self.controlView.totalTimeLabel.text = "00:00"
                    self.controlView.progressView.progress = 0
                    self.play()
                }
    ...
    public func controlView(controlView: BMPlayerControlView, coverViewIsHidden isShow: Bool) {
    }
