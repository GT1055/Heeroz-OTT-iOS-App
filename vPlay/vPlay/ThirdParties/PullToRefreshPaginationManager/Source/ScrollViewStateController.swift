...
public let kDefaultLoaderHeight: CGFloat = 64
...
open class StateConfiguration: NSObject {
	...
	public init(thresholdInitiateLoading: CGFloat, loaderFrame: CGRect, thresholdStartLoading: CGFloat, showDefaultLoader: Bool = true) {
...
open class ScrollViewStateController: NSObject {
    ...
    fileprivate var loadingView = UIActivityIndicatorView(style: .gray)
	...
	fileprivate func handleLoadingCycle(_ offset: CGFloat) {
		...
		if self.scrollView.isDragging {
            ...
            self.loadingView.transform = CGAffineTransform(rotationAngle: (CGFloat(2*Double.pi))*(abs(self.scrollView.contentOffset.y)/128))
